...
def run_live_demo(scenario_key="crypto", open_browser=True, quick=False):
    ...
    print(f"   • Expanded Title: {BOLD}{expansion_data.get('expanded_title', scenario['title'])}{RESET}")
    ...
    case_payload = {
        "title": expansion_data.get("expanded_title", scenario["title"]),
        "division": scenario["division"],
        "change_type": scenario["change_type"],
        "submitter_name": scenario["submitter"],
        "submitter_role": scenario["submitter_role"],
        "what_requester_wants": expansion_data.get("executive_summary", scenario["vague_brief"]),
